...
class AppUI:
    ...
    def _reset_pi_mode_state(self):
        ...
        self.track_vel_mmps = {'X': 5.0, 'Y': 5.0, 'Z': 3.0}

        ENV_OVERRIDE = False  # đổi thành True nếu bạn muốn override bằng ENV
        if ENV_OVERRIDE:
            _tv_base = os.environ.get('TRACK_VEL_MMPS', str(self.track_vel_mmps.get('X', 3.0)))
            self.track_vel_mmps['X'] = float(os.environ.get('TRACK_VEL_X_MMPS', _tv_base))
            self.track_vel_mmps['Y'] = float(os.environ.get('TRACK_VEL_Y_MMPS', _tv_base))
            self.track_vel_mmps['Z'] = float(os.environ.get('TRACK_VEL_Z_MMPS', _tv_base))
        ...
        self._lift_stable = 0
        self._lift_mm = float(os.environ.get('GRASP_LIFT_MM', '65.0'))
    ...
    def _start_lift_after_grasp(self):
        # Move Z toward 0 by _lift_mm (do not go below 0)
        try:
            z_now = float(self.state.pos[2])
            # Example: lift_mm=65
            #   z_now=40  -> z_target=0
            #   z_now=80  -> z_target=15
            z_target = max(0.0, z_now - float(self._lift_mm))
            ...
            self.append_log(f"Lifting Z toward 0 by {self._lift_mm}mm (target Z={z_target:.2f})...", "INFO")
        except Exception as e:
    ...
    def _track_hold_loop(self):
        ...
        try:
                    ...
                    if self._track_hold_active and self._track_hold_axis and (not self.motion_lock):
                        ...
                        # Pick per-axis velocity; fallback to scalar for old configs
                        try:
                            tv = getattr(self, 'track_vel_mmps', 1.0)
                            v = float(tv.get(axis, tv.get('X', 1.0)) if isinstance(tv, dict) else float(tv))
                        except Exception:
                            v = 1.0
                        v = abs(v)
                        if sign == "-":
